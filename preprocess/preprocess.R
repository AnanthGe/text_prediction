...
#'
#'
#' Design Considerations: Cleaning needs to be done parallelly



#' Process the complete corpus of text
#' using Split--> Apply --> Combine techniques
#'

clean.corpus <-
  function(raw.data) {

    #Add group numbers to the lines of text in the corpus
    group.numbers <- get.group.numbers(number.workers = number.workers,
                                       length.obs = length(raw.data))

    text.corpus <- as.data.frame(group.numbers)
    text.corpus$txt <- raw.data
    ...
    text.corpus.groups <-
      ...
      cluster_library("stringi") %>%
      cluster_library("stringr") %>%
      cluster_library("tm") %>%
      cluster_assign_value("clean.line", clean.line) %>%
      cluster_assign_value("removeGenitives", removeGenitives) %>%
      cluster_assign_value("stemGen", stemGen) %>%
      cluster_assign_value("no.stem", no.stem) %>%
      cluster_assign_value("stemText", stemText)


    t1 <- Sys.time()
    paral <- text.corpus.groups %>%
...
clean.line <-
  function(txt) {
    ...
    txt <- gsub("(\\#[a-zA-Záé�???óúàèìòùäöüßñ0-9']+)", " <HASH> ", txt)
    #replace twitter-like names with special marker
    txt <- gsub("\\@\\w*[a-zA-Záé�???óúàèìòùäöüßñ0-9]+\\w*", " <TW> ", txt)
    ...
    txt <- gsub("(^|\\.|\\s)([a-záé�???óúàèìòùäöüßñ<>]+)\\.'([a-z])(\\s|\\.|$)", " \\2'\\3 ", txt)
    ...
    txt <- gsub("[^0-9A-Za-záé�???óúàèìòùäöüßñ.'<>]+", " ", txt)
    ...
    txt <- gsub("^'", "", txt)
    txt <- gsub("(^|\\s)'([a-záé�???óúàèìòùäöüßñ<>]+)'?(\\s|$)", "\\1\\2 ", txt)
    txt <- gsub("(\\s)?([a-záé�???óúàèìòùäöüßñ<>]+)'(\\s|$)", "\\1\\2 ", txt)
...
removeGenitives <- function(txt) {
  unlist(lapply(strsplit(txt, " "), function(st)  paste(sapply(st, stemGen), collapse = " ")))
}
...
#stem a piece of text using Snowball algorithm for english
stemText <- function(txt) {
  unlist( lapply( strsplit(txt, " "),
                  function(st) paste( sapply(st, stemDocument), collapse=" ") ))
}
